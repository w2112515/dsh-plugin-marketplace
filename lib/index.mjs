...
var MarketplaceProfileOperations = class {
	...
	*/
	snapshot() {
		...
		for (const state of catalog?.entries.map((entry) => pluginState(this.options.runtime, manifest, entry)).filter((state) => state.state !== "not-installed") ?? []) {
			const key = state.packageName ?? state.repositoryId;
			const existing = byPackage.get(key);
			if (existing === void 0 || existing.installedSpec !== existing.catalogSpec && state.installedSpec === state.catalogSpec) byPackage.set(key, state);
		}
		return {
