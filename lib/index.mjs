...
function queryMarketplaceCatalog(view, request) {
	...
	const selected = entries.filter((entry) => request.installability === "all" || entry.installability === request.installability).map((entry) => ({
		entry,
		relevance: relevance(entry, words)
	...
	*/
	snapshot() {
		...
		return {
			...
			plugins: catalog?.entries.map((entry) => pluginState(this.options.runtime, manifest, entry)).filter((state) => state.state !== "not-installed") ?? []
		};
	}
	/**
