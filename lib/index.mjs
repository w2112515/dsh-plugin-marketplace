...
async function runPnpm(args, cwd, signal) {
	try {
		return {
			exitCode: (await execa("pnpm", args, {
				cwd,
...
var MarketplaceProfileOperations = class {
	...
	*/
	plan(request) {
		...
		if (state === void 0) return emptyPlan(request, this.options.runtime.profileName, "catalog-entry-missing", entry);
