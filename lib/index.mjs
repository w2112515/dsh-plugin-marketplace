...
function listMarketplacePacks(view) {
	const packs = (view.catalog?.packs ?? []).filter(isPublicMarketplacePack).sort((left, right) => right.stars - left.stars || compareText(left.repository.fullName, right.repository.fullName)).map(packSummary);
	return {
		digest: view.catalog?.integrity.digest ?? "",
...
function isGrantLineFor(line, packageName) {
...
*/
function grantBuildConsent(text, packageName, sourceRef) {
	const grantLine = `  ${quoteYamlKey(`${packageName}@${sourceRef}`)}: true`;
	const lines = text.split("\n");
	if (lines.findIndex((line) => /^allowBuilds:\s*$/.test(line)) === -1) {
...
function revokeBuildConsent(text, packageName) {
	...
	if (at !== -1) {
		...
		if (next === void 0 || !next.startsWith(" ") && !next.startsWith("	")) kept.splice(at, 1);
	}
	return kept.join("\n");
}
...
var MarketplaceProfileOperations = class {
	...
	*/
	...
	async run(stored, signal) {
		...
		if (plan.action !== "remove" && plan.requiresScripts) try {
			const workspacePath = join(this.options.runtime.dir, WORKSPACE_FILE);
			await writeFileAtomic(workspacePath, grantBuildConsent(await readFile(workspacePath, "utf8"), packageName, plan.sourceRef), {
				mode: 384,
				dirMode: 448
			});
		} catch {
		...
		try {
			...
			if (plan.action === "remove") {
				...
				try {
					...
					if (next !== current) await writeFileAtomic(workspacePath, next, {
						mode: 384,
						dirMode: 448
					});
				} catch {}
