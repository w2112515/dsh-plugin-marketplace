...
//#region src/category-vocabulary.ts
...
*/
const MARKETPLACE_CATEGORY_VOCABULARY = [
	{
		slug: "theme",
		zh: "主题",
		en: "Themes",
...
function marketplaceQueryWords(query) {
...
function marketplaceWordMatcher(word) {
	const lowered = word.toLocaleLowerCase();
	if ([...word].length === 1 && CJK_CHAR.test(word)) return (field) => field.toLocaleLowerCase() === lowered;
...
function vocabularyAliases(item) {
	return [
		item.slug,
		item.zh,
		item.en.toLocaleLowerCase(),
		...item.aliases
	];
}
function marketplaceQueryCategoryAlias(word) {
	for (const item of MARKETPLACE_CATEGORY_VOCABULARY) for (const alias of vocabularyAliases(item)) {
...
function deriveMarketplaceCategory(entry) {
	...
	for (const category of MARKETPLACE_CATEGORY_PRIORITY) {
		...
		if (vocab !== void 0 && vocab.tokens.some((token) => tokens.has(token))) return category;
	}
	const haystack = [
		...entry.topics,
		...entry.keywords,
	...
	for (const category of MARKETPLACE_CATEGORY_PRIORITY) {
		const vocab = MARKETPLACE_CATEGORY_VOCABULARY.find((item) => item.slug === category);
		if (vocab !== void 0 && vocab.needles.some((needle) => haystack.includes(needle))) return category;
	}
	return null;
}
...
function summary(entry, nowMs, issueUrl, category = deriveMarketplaceCategory(entry)) {
...
function relevance(entry, category, words, matchers) {
	...
	for (const item of matchers) {
		...
		for (const [field, weight] of fields) if (field.toLocaleLowerCase() === item.word) {
			score += weight * 2;
			matched = true;
...
function queryMarketplaceCatalog(view, request, now) {
	...
	return {
		...
		page,
		pageCount,
		items: selected.slice(offset, offset + 50).map((item) => summary(item.entry, nowMs, view.catalog?.ratings?.issueUrl ?? null, item.category)),
		error: view.error
	};
}
...
const CATEGORY_VALUES = [...MARKETPLACE_CATEGORY_PRIORITY, "uncategorized"];
const SORT_VALUES = [
	"recommended",
	"stars",
	"recently-updated",
	...
	async persist(record) {
		try {
			...
			this.error = {
				...
				message: "The refreshed marketplace catalog could not be saved."
			};
		}
	}
...
//#endregion
//#region src/index.ts
const DEFAULT_MARKETPLACE_CATALOG_URL = "https://w2112515.github.io/dsh-plugin-marketplace/plugin-marketplace/catalog-v1.json";
...
function verifySameOrigin(req) {
	...
	if (host === void 0) throw new ApiFailure(403, "origin-denied", "The marketplace API accepts only same-origin WebUI requests.");
	if (originUrl === null) {
		if (!sameFetch) throw new ApiFailure(403, "origin-denied", "The marketplace API accepts only same-origin WebUI requests.");
		return;
	}
	if (originUrl.protocol !== "http:" && originUrl.protocol !== "https:" || originUrl.host !== host) throw new ApiFailure(403, "origin-denied", "The marketplace API accepts only same-origin WebUI requests.");
}
...
//#endregion
export { Config, DEFAULT_MARKETPLACE_CATALOG_URL, MarketplaceCatalogParseError, apply, computeMarketplaceCatalogDigest, inject, name, parseMarketplaceCatalogText, sealMarketplaceCatalog };
