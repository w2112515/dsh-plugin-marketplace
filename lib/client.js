window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		const MARKETPLACE_PACK_QUERY_ALIASES = [
			"整合方案",
			"整合",
			"solution packs",
			"packs"
		];
		function marketplaceCategoryLocaleEntries(locale) {
			return Object.fromEntries(MARKETPLACE_CATEGORY_VOCABULARY.map((item) => [`category.${item.slug}`, item[locale]]));
		}
		...
		function marketplaceFieldMatchesWord(field, word) {
			return marketplaceWordMatcher(word)(field);
		}
		function marketplaceQueryIsPackAlias(word) {
			return MARKETPLACE_PACK_QUERY_ALIASES.includes(word);
		}
		/**
		...
		*/
		function marketplaceTextsMatchQuery(texts, query, options = {}) {
			const trimmed = query.trim().toLocaleLowerCase();
			if (trimmed.length === 0) return true;
			if (options.packs === true && marketplaceQueryIsPackAlias(trimmed)) return true;
			return marketplaceQueryWords(query).every((word) => {
		...
		*/
		...
		function PluginMarketplaceSettingsTab({ bootstrap, list, detail, refresh, operationSnapshot, installed, packs, packDetail, plan, execute, activateTab, dateLocale, t }) {
			...
			const request = (0, react.useMemo)(() => requestFor(debouncedQuery, category, filter, sort, page), [
				debouncedQuery,
				category,
				filter,
				sort,
				...
				return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					...
					children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							failureReason ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
								className: PluginMarketplaceSettingsTab_module_css_default.failureDetail,
			...
			const visibleInstalled = sortInstalledItems(installedCategoryFilter === "all" ? installedItems : installedItems.filter((item) => installedCategory(item) === installedCategoryFilter), installedSort).filter((item) => marketplaceTextsMatchQuery([
				item.plugin?.name ?? "",
			...
			const visibleExternal = (installedModel !== null && (installedCategoryFilter === "all" || installedCategoryFilter === "uncategorized") ? installedModel.external : []).filter((item) => marketplaceTextsMatchQuery([item.packageName, item.installedSpec ?? ""], query));
			const anyRestartPending = (profile?.plugins ?? []).some((state) => isRestartPending(state.state));
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: PluginMarketplaceSettingsTab_module_css_default.section,
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					installedModel !== null ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
						...
						visibleInstalled.length === 0 && visibleExternal.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
							className: PluginMarketplaceSettingsTab_module_css_default.status,
							children: query ? t("state.emptySearch") : t("installed.empty")
						}) : null,
		...
		const en = {
			...
			search: "Search names, categories (e.g. Security), authors, or keywords",
